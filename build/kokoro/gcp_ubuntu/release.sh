...
docker cp "$container_id":/usr/lib/x86_64-linux-gnu/libstdc++.so.6 $OUTPUT_DIR
...
tar -C "${OUTPUT_DIR}" -czf "${OUTPUT_DIR}"/cloud-spanner-emulator_linux_${CLOUD_SPANNER_EMULATOR_MACHINE_ARCH}-"${EMULATOR_VERSION}".tar.gz gateway_main emulator_main THIRD_PARTY_NOTICES.txt libstdc++.so.6
