...
googlesql::TypeKind GetMinMaxTypeKind(absl::Span<const googlesql::Value> args) {
  googlesql::TypeKind type_kind = googlesql::TypeKind::TYPE_UINT64;
...
absl::Status ValidateMinMaxAndGranularity(
    ...
    std::function<T(const googlesql::Value)> get_value) {
...
absl::Status ValidateTokenizeNumberOptionalParameters(
  ...
  if (!absl::EqualsIgnoreCase(algorithm, "floatingpoint")) {
    ...
    googlesql::TypeKind type_kind = GetMinMaxTypeKind(args);
    if (type_kind == googlesql::TypeKind::TYPE_FLOAT ||
        type_kind == googlesql::TypeKind::TYPE_DOUBLE) {
      return ValidateMinMaxAndGranularity<double>(
          args, [](const googlesql::Value& value) -> double {
            return value.double_value();
          });
    } else {
      return ValidateMinMaxAndGranularity<int64_t>(
          args, [](const googlesql::Value& value) -> int64_t {
            return value.int64_value();
          });
    }
  } else {
    const googlesql::Type* type = args[0].type();
    if (type->IsArray()) {
...
// correspondent SEARCH function. So only add the tokenize function name into
// the tokenlist to block it being used by SEARCH or SEARCH_SUBSTRING function.
absl::StatusOr<googlesql::Value> NumericTokenizer::Tokenize(
    absl::Span<const googlesql::Value> args) {
  GOOGLESQL_RETURN_IF_ERROR(ValidateTokenizeNumberOptionalParameters(args));
