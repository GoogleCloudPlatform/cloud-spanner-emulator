...
googlesql::FunctionOptions QueryableUdf::CreateFunctionOptions(
  ...
  auto evaluator = [=](absl::Span<const googlesql::Value> args)
    ...
    GOOGLESQL_RETURN_IF_ERROR(googlesql::AnalyzeExpression(udf->body(), options, catalog,
                                                 type_factory, &output));
    ...
    googlesql::PreparedExpression expr(resolved_expr.get(), evaluator_options);
