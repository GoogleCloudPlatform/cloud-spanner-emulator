...
class Index;
class Table;
inline constexpr absl::string_view kDeleteAction = "ON DELETE";

// Foreign key relationship between two tables.
class ForeignKey : public SchemaNode {
 public:
  // Describes what action to take when the row from the referenced table is
  // deleted or updated.
  enum class Action {
    kActionUnspecified,
    kNoAction,
  ...
  static absl::string_view ActionName(ForeignKey::Action action) {
    switch (action) {
    ...
    ABSL_LOG(ERROR) << "Unexpected value for ForeignKey::Action enum: "
