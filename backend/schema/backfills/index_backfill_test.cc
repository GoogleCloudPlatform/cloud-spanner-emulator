...
absl::Status UpdateSchema(
  ...
  GOOGLESQL_RETURN_IF_ERROR(db_info.database->UpdateSchema(
...
absl::Status InsertValues(DbInfo& db_info, std::string table,
                          ...
                          std::vector<ValueList> values) {
  GOOGLESQL_ASSIGN_OR_RETURN(std::unique_ptr<ReadWriteTransaction> txn,
...
TEST(BackfillTest, SchemaChange) {
  GOOGLESQL_ASSERT_OK_AND_ASSIGN(DbInfo db_info, CreateTestDb({kCreateTestTable}));
  GOOGLESQL_ASSERT_OK(UpdateSchema(db_info, {R"(
...
TEST(BackfillTest, BasicTableValues) {
  ...
  GOOGLESQL_ASSERT_OK(InsertValues(db_info, table, columns, values_write));
...
TEST(BackfillTest, BasicIndexValues) {
  GOOGLESQL_ASSERT_OK_AND_ASSIGN(DbInfo db_info, CreateTestDb({kCreateTestTable}));
  GOOGLESQL_ASSERT_OK(InsertValues(
      db_info, "TestTable",
...
TEST(BackfillTest, IndexNullValues) {
  GOOGLESQL_ASSERT_OK_AND_ASSIGN(DbInfo db_info, CreateTestDb({kCreateTestTable}));
  GOOGLESQL_ASSERT_OK(InsertValues(db_info, "TestTable", {"int64_col"},
                         {{Int64(1)}, {Int64(2)}}));
