...
/* SPANGRES BEGIN */
// Make this a thread-local.  This is a freelist for palloc()'ed blocks.
// Spangres makes PG allocators thread-local to avoid race conditions (since
// palloc() doesn't use mutexes).
							  ...
							  Size maxBlockSize)
{
	 ...
	 */
	/* SPANGRES BEGIN */
	// Use Spangres's wrapped allocator method, which first checks with Spanner's
	// reservation manager to see whether an allocation is permitted (or would
	// exceed the current operation's memory budget) before allocating.
	set = (AllocSet) reserved_alloc(firstBlockSize);
	/* SPANGRES END */
	if (set == NULL)
	{
		if (TopMemoryContext)
...
AllocSetAlloc(MemoryContext context, Size size)
{
	...
	if (size > set->allocChunkLimit)
	{
		...
		block = (AllocBlock) reserved_alloc(blksize);
		if (block == NULL)
			return NULL;
...
AllocSetFree(MemoryContext context, void *pointer)
{
	...
	if (chunk->size > set->allocChunkLimit)
	{
		...
		/*
		 * Try to verify that we have a valid block pointer: it should
...
AllocSetRealloc(MemoryContext context, void *pointer, Size size)
{
	...
	if (oldsize > set->allocChunkLimit)
	{
		...
		block = (AllocBlock) reserved_realloc(
			block, oldsize + ALLOC_BLOCKHDRSZ + ALLOC_CHUNKHDRSZ, blksize);
...
/* SPANGRES BEGIN */
// Add code, called from our shims, that knows how to free all pages in a
// freelist.  This logic is inlined in functions above, but we need to be able
// to call it directly in order to manually free thread-local allocations at
// the end of a thread.  (PG doesn't have threads, and appears to just
// leak these allocations at the end of a process since the kernel is about to
// release them anyway in that case?)
void AsetDeleteFreelists(void)
{
	const size_t freelist_size = \
		sizeof(context_freelists)/sizeof(context_freelists[0]);

	for (size_t freelist_idx = 0; freelist_idx < freelist_size; ++freelist_idx) {
		AllocSetFreeList *freelist = &context_freelists[freelist_idx];
