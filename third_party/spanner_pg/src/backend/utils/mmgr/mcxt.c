 ...
 */
/* SPANGRES BEGIN */
// Make this thread-local for Spangres.
...
/* SPANGRES BEGIN */
// We've broken the ability to reset children without also deleting them. To
// work without TopMemoryContext, we simply allocate new contexts inside of
// their parents. Deleting a tree works fine, but you can no longer reset a
// context and not also delete its children. This doesn't appear to be in use
// anywhere (confirmed by commenting out this function) but if a future update
// needs that functionality, we'll add it back in and find a different solution.
// /*
