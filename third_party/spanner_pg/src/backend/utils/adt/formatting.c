	...
	for (n = node, s = in; n->type != NODE_TYPE_END && *s != '\0'; n++)
	{
		...
		switch (n->key->id)
		{
			...
			case DCH_ID:
				...
				// Protect against signed integer overflow on out->d when parsing DCH_ID.
				if (pg_add_s32_overflow(out->d, 1, &out->d)) {
					ereturn(escontext,,
							(errcode(ERRCODE_DATETIME_VALUE_OUT_OF_RANGE),
...
// SPANGRES BEGIN
// Frees the memory for thread-local date/time cache `DCHCache` on this thread.
//
// This function has been added here, instead of adding a shim,
// because the cache is internal to this file. Rather than breaking
// encapsulation, we thought better to add the function directly here
// than to copy all the functions that required it into a separate shim file.
void CleanupDateTimeCache() {
	for (int i = 0; i < n_DCHCache; i++) {
		pfree(DCHCache[i]);
		DCHCache[i] = NULL;
	}
...
// Frees the memory for the thread-local number cache `NUMCache` on this thread.
void CleanupNumberCache() {
	...
	*fsec = 0;
	...
	if (tmfc.ddd && (tm->tm_mon <= 1 || tm->tm_mday <= 1))
	{
		...
		if (tmfc.mode == FROM_CHAR_DATE_ISOWEEK)
		{
			...
			// `j0 + tmfc.ddd`
			int tmp;
			if (pg_add_s32_overflow(j0, tmfc.ddd, &tmp)) {
				ereport(ERROR,
								(errcode(ERRCODE_DATETIME_VALUE_OUT_OF_RANGE),
								errmsg("timestamp out of range")));
			}
			j2date(tmp, &tm->tm_year, &tm->tm_mon, &tm->tm_mday);
			// SPANGRES END
			fmask |= DTK_DATE_M;
		}
		else
...
float4_to_char(PG_FUNCTION_ARGS)
{
	...
	if (IS_ROMAN(&Num)) {
		// SPANGRES BEGIN
		// Sanitizer complains about unsafe float to int conversion, so we make
		// sure to check for boundaries before converting.
		float rounded_value = rint(value);
		int int_value = 0;
...
float8_to_char(PG_FUNCTION_ARGS)
{
	...
	if (IS_ROMAN(&Num)) {
		// SPANGRES BEGIN
		// Sanitizer complains about unsafe double to int conversion, so we make
		// sure to check for boundaries before converting.
