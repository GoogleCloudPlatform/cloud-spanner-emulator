...
date2j(int year, int month, int day)
{
	...
	int century_tmp = year / 4 - century + century / 4;
	int month_tmp = 7834 * month / 256 + day;

	PG_MUL_S32_RETURN_IF_OVERFLOW(year, 365, julian);
			...
			case DTK_DATE:
				...
				else
				{
					if (isdigit((unsigned char) *field[i]))
					{
						...
						/*
						 * Should not get here and fail. Validity check only...
						 */
						if ((cp = strchr(field[i], '-')) == NULL)
							return DTERR_BAD_FORMAT;
					 ...
					 DateTimeErrorExtra *extra)
{
	...
	/* Just some validity checks to prevent indexing off into nowhere */
