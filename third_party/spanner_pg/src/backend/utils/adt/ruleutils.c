...
/* SPANGRES BEGIN */
// These flags are redefined in catalog_shim.h

 /* Indent counts */
#define PRETTYINDENT_STD               POSTGRES_PRETTYINDENT_STD
#define PRETTYINDENT_JOIN              POSTGRES_PRETTYINDENT_JOIN
#define PRETTYINDENT_VAR               POSTGRES_PRETTYINDENT_VAR
 
#define PRETTYINDENT_LIMIT POSTGRES_PRETTYINDENT_LIMIT /* wrap limit */
 
 /* Pretty flags */
#define PRETTYFLAG_PAREN               POSTGRES_PRETTYFLAG_PAREN
#define PRETTYFLAG_INDENT              POSTGRES_PRETTYFLAG_INDENT
#define PRETTYFLAG_SCHEMA              POSTGRES_PRETTYFLAG_SCHEMA
/* SPANGRES END */
...
} deparse_context_UNUSED_SPANGRES;
...
typedef struct
{
	...
	int			numargs;
	char	  **argnames;
} deparse_namespace_UNUSED_SPANGRES;
...
} deparse_columns_UNUSED_SPANGRES;
...
static void set_rtable_names(deparse_namespace *dpns, List *parent_namespaces,
							 Bitmapset *rels_used);
// SPANGRES BEGIN
// We've made this function non-static to call it from deparser
...
static void set_using_names(deparse_namespace *dpns, Node *jtnode,
							List *parentUsing);
static void set_relation_column_names_UNUSED_SPANGRES(deparse_namespace *dpns,
									  RangeTblEntry *rte,
...
static void get_update_query_def_UNUSED_SPANGRES(Query *query, deparse_context *context,
								 bool colNamesVisible);
void get_update_query_targetlist_def(Query *query, List *targetList,
											deparse_context *context,
											RangeTblEntry *rte);
static void get_delete_query_def_UNUSED_SPANGRES(Query *query, deparse_context *context,
								 bool colNamesVisible);
static void get_utility_query_def_UNUSED_SPANGRES(Query *query, deparse_context *context);
static void get_basic_select_query(Query *query, deparse_context *context,
								   TupleDesc resultDesc, bool colNamesVisible);
// SPANGRES BEGIN
// We've made this function non-static to call them from catalog_shim.
void get_target_list(List *targetList, deparse_context *context,
							TupleDesc resultDesc, bool colNamesVisible);
// SPANGRES END
static void get_setop_query_UNUSED_SPANGRES(Node *setOp, Query *query,
							deparse_context *context,
							TupleDesc resultDesc, bool colNamesVisible);
...
static void get_oper_expr_UNUSED_SPANGRES(OpExpr *expr, deparse_context *context);
...
static char *generate_relation_name_UNUSED_SPANGRES(Oid relid, List *namespaces);
static char *generate_qualified_relation_name(Oid relid);
static char *generate_function_name_UNUSED_SPANGRES(Oid funcid, int nargs,
									List *argnames, Oid *argtypes,
									bool has_variadic, bool *use_variadic_p,
									ParseExprKind special_exprkind);
static char *generate_operator_name_UNUSED_SPANGRES(Oid operid, Oid arg1, Oid arg2);
...
#define only_marker_UNUSED_SPANGRES(rte) ((rte)->inh ? "" : "ONLY ")
 ...
 */
static void
set_relation_column_names_UNUSED_SPANGRES(deparse_namespace *dpns, RangeTblEntry *rte,
						  deparse_columns *colinfo)
{
	int			ncolumns;
	char	  **real_colnames;
...
/* SPANGRES END */
{
	...
	/* SPANGRES BEGIN */
	get_hint_list_def(query->statementHints, &context, /*statement=*/true);
	/* SPANGRES END */
...
/* SPANGRES END */
{
	...
	foreach(l, targetList)
	{
		...
		if (POSTGRES_PRETTY_INDENT(context) && context->wrapColumn >= 0)
		{
			int			leading_nl_pos;
...
static void
get_setop_query_UNUSED_SPANGRES(Node *setOp, Query *query, deparse_context *context,
				TupleDesc resultDesc, bool colNamesVisible)
{
	StringInfo	buf = context->buf;
	bool		need_paren;
						 ...
						 deparse_context *context)
{
	...
	else
	{
		 ...
		 * POSTGRES_PRETTY_PAREN is off, since those are the ones in danger of
		 * misparsing. For other expressions we need to force them only if
		 * POSTGRES_PRETTY_PAREN is on, since otherwise the expression will output them
		 * itself. (We can't skip the parens.)
		 */
		bool		need_paren = (POSTGRES_PRETTY_PAREN(context)
								  || IsA(expr, FuncExpr)
								  || IsA(expr, Aggref)
								  || IsA(expr, WindowFunc));
 ...
 */
static void
get_update_query_def_UNUSED_SPANGRES(Query *query, deparse_context *context,
					 bool colNamesVisible)
{
	StringInfo	buf = context->buf;
	RangeTblEntry *rte;
...
/* ----------
 * get_delete_query_def_spangres			- Parse back a DELETE parsetree
 * ----------
 */
static void
get_delete_query_def_UNUSED_SPANGRES(Query *query, deparse_context *context,
					 bool colNamesVisible)
{
	StringInfo	buf = context->buf;
	RangeTblEntry *rte;
 ...
 */
static void
get_utility_query_def_UNUSED_SPANGRES(Query *query, deparse_context *context)
{
	StringInfo	buf = context->buf;
 ...
 */
/* SPANGRES BEGIN */
// Remove 'static' modifier so this method can be called from elsewhere
void
/* SPANGRES END */
...
/* SPANGRES END */
	...
	need_paren = POSTGRES_PRETTY_PAREN(context) &&
 ...
 * to look like a function call, wrap it in a placeholder CAST() expression, which
...
/*
 * get_oper_expr_spangres			- Parse back an OpExpr node
 */
static void
get_oper_expr_UNUSED_SPANGRES(OpExpr *expr, deparse_context *context)
{
	StringInfo	buf = context->buf;
	Oid			opno = expr->opno;
	List	   *args = expr->args;
			  ...
			  bool showimplicit)
{
	...
	/* SPANGRES BEGIN */
	get_hint_list_def(expr->functionHints, context, /*statement=*/false);
	/* SPANGRES END */
}
			 ...
			 Aggref *original_aggref)
{
	...
	if (AGGKIND_IS_ORDERED_SET(aggref->aggkind))
	{
		...
		/* SPANGRES BEGIN */
		appendStringInfoString(buf, ")");
		get_hint_list_def(aggref->functionHints, context, /*statement=*/false);
		/* SPANGRES END */
...
get_windowfunc_expr(WindowFunc *wfunc, deparse_context *context)
{
	...
	/* SPANGRES BEGIN */
	appendStringInfoString(buf, ")");
	get_hint_list_def(wfunc->functionHints, context, /*statement=*/false);
	/* SPANGRES END */
...
get_from_clause_item(Node *jtnode, Query *query, deparse_context *context)
{
			...
			case RTE_RELATION:
				...
				appendStringInfo(buf, "%s%s",
								 ...
								 generate_relation_name(rte->relid,
														context->namespaces));
				/* SPANGRES BEGIN */
				get_hint_list_def(rte->tableHints, context, /*statement=*/false);
				/* SPANGRES END */
				break;
			case RTE_SUBQUERY:
				/* Subquery RTE */
	...
	else if (IsA(jtnode, JoinExpr))
	{
		...
		need_paren_on_right = POSTGRES_PRETTY_PAREN(context) &&
			!IsA(j->rarg, RangeTblRef) &&
			!(IsA(j->rarg, JoinExpr) && ((JoinExpr *) j->rarg)->alias != NULL);

		if (!POSTGRES_PRETTY_PAREN(context) || j->alias != NULL)
			appendStringInfoChar(buf, '(');
		...
		/* SPANGRES BEGIN */
		get_hint_list_def(j->joinHints, context, /*statement=*/false);
		/* SPANGRES END */
 ...
 */
static char *
generate_relation_name_UNUSED_SPANGRES(Oid relid, List *namespaces)
{
	HeapTuple	tp;
 ...
 */
static char *
generate_function_name_UNUSED_SPANGRES(Oid funcid, int nargs, List *argnames, Oid *argtypes,
					   bool has_variadic, bool *use_variadic_p,
					   ParseExprKind special_exprkind)
{
	char	   *result;
 ...
 */
static char *
generate_operator_name_UNUSED_SPANGRES(Oid operid, Oid arg1, Oid arg2)
{
	StringInfoData buf;
	HeapTuple	opertup;
