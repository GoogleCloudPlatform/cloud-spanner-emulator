...
slow:
	...
	if (neg)
	{
		...
		if (tmp > (uint16) (-(PG_INT16_MIN + 1)) + 1)
			goto out_of_range;
		// SPANGRES BEGIN
		if (tmp == (uint16)(-(PG_INT16_MIN + 1)) + 1) return PG_INT16_MIN;
		// SPANGRES END
		return -((int16) tmp);
	}
...
pg_strtoint32_safe(const char *s, Node *escontext)
{
	...
	if (neg)
	{
		...
		if (unlikely(tmp > (uint32) (-(PG_INT32_MIN + 1)) + 1))
			goto out_of_range;
		// SPANGRES BEGIN
		if (tmp == (uint32)(-(PG_INT32_MIN + 1)) + 1) return PG_INT32_MIN;
		// SPANGRES END
		return -((int32) tmp);
	}
...
pg_strtoint64_safe(const char *s, Node *escontext)
{
	...
	if (neg)
	{
		...
		if (unlikely(tmp > (uint64) (-(PG_INT64_MIN + 1)) + 1))
			goto out_of_range;
		// SPANGRES BEGIN
		if (tmp == (uint64)(-(PG_INT64_MIN + 1)) + 1) return PG_INT64_MIN;
		// SPANGRES END
		return -((int64) tmp);
	}
