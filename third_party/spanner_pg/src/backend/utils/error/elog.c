 ...
 *-------------------------------------------------------------------------
 */
// Portions derived from source copyright YugaByte, Inc. in accordance with the
// following notice:
//--------------------------------------------------------------------------------------------------
...
errfinish(const char *filename, int lineno, const char *funcname)
{
	...
	if (elevel == ERROR)
	{
		 ...
		 * execute in a reasonably valid state.
		 *
...
// Spangres error-handling functions to bypass the PostgreSQL error mechanism
// and pass error information directly to Spangres.

// Function to initialize thread-locals that we need for error reporting
void spangres_errstart(void) {
	ErrorData *edata = &errordata[0];
	memset(edata, 0, sizeof(*edata));

	if (CurrentMemoryContext == NULL) {
		// All errors will fail to allocate memory for their error message if
		// the CurrentMemoryContext is not initialized. Write a generic message
		// to stderr and exit early.
		write_stderr(
		...
		edata->message_id =
			"error occurred before error message processing is available";
		spangres_throw_exception(edata);
	}
}

// Placeholder function to force evaluation of the argument pack in an ereport call.
// This replaces the 'errfinish rest' in the PostgreSQL evaluation framework.
void spangres_evaluate(int ignored, ...) {
...
extern void spangres_ereport(int elevel, const char* msg,...)
{
  // spangres_ereport() calls things that may themselves try to report errors.
	...
	static __thread bool in_spangres_ereport = false;

	// We're being called from code that constructs errors in shared thread-local
	// storage
	ErrorData *edata = &errordata[0];

	const char *formatted_msg = NULL;

	// `msg` is typically un-set if we got here from ereport().
	// In which case `formatted_msg` should have already been set by a call
	// to `errmsg()`.  (Or not, in which case it is deliberately null and the
	// exception handler should interpret this as an error with no error message.)
	if (msg)
	{
		// Use the unformatted message as a default.
		formatted_msg = msg;

		if (CurrentMemoryContext && !in_spangres_ereport)
		{
			in_spangres_ereport = true;

			va_list args;
			va_start(args, msg);

			// pvsprintf() ereport()'s if the format-string is invalid or if it can't
			// allocate enough memory for the return buffer.
			// `InSpangresEreport` guards this block so that we don't end up in an
			// infinite loop trying and failing to pvsprintf().
			formatted_msg = pvsprintf(msg, args);
			edata->message_id = msg;
			va_end(args);

			in_spangres_ereport = false;
		}
	}

	// Make a copy in case formatted_msg points to a string literal.
	// (edata->message is a char* for unknown historical reasons that are hard
	// to revert now.)
	if (formatted_msg) {
		// Include null terminator
		size_t msg_mem_length = strlen(formatted_msg) + 1;

		// Place the copy on the stack so we don't need a working allocator.
		// This is safe because spangres_throw_exception() immediately copies
		// it into a std::string on the heap.
		char *message_buf = alloca(msg_mem_length);
		memcpy(message_buf, formatted_msg, msg_mem_length);
	...
	// PostgreSQL has many options about when to throw and when to log
	// errors. We'll offer a simplified set: throw if elevel >= ERROR
	// (matches PostgreSQL) and log otherwise--this doesn't match PostgreSQL but
	// preserves error information at the risk of extra verbosity.
	if (edata->elevel < ERROR) {
		spangres_log_exception(edata);
	} else {
		spangres_throw_exception(edata);
  }
}

// Short circuit error processing when we're using Spangres' error processing
// since we don't set up the standard PostgreSQL error context. Since we don't
// set up the PostgreSQL error processing structures, it's not safe to complete
// error processing.
#define SPANGRES_SHORT_CIRCUIT() return 0
...
{
	// Spangres doesn't currently support error nesting
	ErrorData  *edata = &errordata[0];
  edata->sqlerrcode = sqlerrcode;
  SPANGRES_SHORT_CIRCUIT();
	edata = &errordata[errordata_stack_depth];
...
{
  // Spangres shouldn't get these errors because we don't use the filesystem.
	// Spangres doesn't currently support error nesting
	ErrorData  *edata = &errordata[0];
  edata->sqlerrcode = ERRCODE_INTERNAL_ERROR;
  SPANGRES_SHORT_CIRCUIT();
...
{
  // Spangres shouldn't get these errors because we don't make requests on the network.
	// Spangres doesn't currently support error nesting
	ErrorData  *edata = &errordata[0];
  edata->sqlerrcode = ERRCODE_INTERNAL_ERROR;
  SPANGRES_SHORT_CIRCUIT();
...
errmsg(const char *fmt,...)
{
	...
	// If they do, keep track so we can bail rather than entering an infinite loop
	// when we encouter a double-fault.
	static __thread bool in_errmsg = false;

	va_list args;
	va_start(args, fmt);

	// This could double-fault, as in spangres_ereport().
	// Put guards around it in case it does.
	if (in_errmsg)  {
		write_stderr("errmsg failed to print the error message\n");
		in_errmsg = false;
	} else {
		in_errmsg = true;
		edata->message = pvsprintf(fmt, args);
...
errmsg_internal(const char *fmt,...)
{
	...
	// Put guards around it in case it does.
	if (in_errmsg)  {
		write_stderr("errmsg_internal failed to print the error message\n");
		in_errmsg = false;
	} else {
		in_errmsg = true;
		edata->message = pvsprintf(fmt, args);
			  ...
			  unsigned long n,...)
{
	...
	// Put guards around it in case it does.
	if (in_errmsg)  {
		write_stderr("errmsg_plural failed to print the error message\n");
		in_errmsg = false;
	} else {
		in_errmsg = true;
		edata->message = pvsprintf(n == 1 ? fmt_singular : fmt_plural, args);
...
errdetail(const char *fmt,...)
{
	...
	// If they do, keep track so we can bail rather than entering an infinite loop
	// when we encouter a double-fault.
	static __thread bool in_errdetail = false;
	...
	// Put guards around it in case it does.
	if (in_errdetail)  {
		write_stderr("errdetail failed to print the error message\n");
		in_errdetail = false;
	} else {
		in_errdetail = true;
		edata->detail = pvsprintf(fmt, args);
		in_errdetail = false;
	}
...
errdetail_internal(const char *fmt,...)
{
	...
	// Put guards around it in case it does.
	if (in_errdetail)  {
		write_stderr("errdetail_internal failed to print the error message\n");
		in_errdetail = false;
	} else {
		in_errdetail = true;
		edata->detail = pvsprintf(fmt, args);
...
errdetail_log(const char *fmt,...)
{
	...
	// Put guards around it in case it does.
	if (in_errdetail)  {
		write_stderr("errdetail_log failed to print the error message\n");
		in_errdetail = false;
	} else {
		in_errdetail = true;
		edata->detail_log = pvsprintf(fmt, args);
		in_errdetail = false;
	}
					 ...
					 unsigned long n,...)
{
	...
	// Put guards around it in case it does.
	if (in_errdetail)  {
		write_stderr("errdetail_log_plural failed to print the error message\n");
		in_errdetail = false;
	} else {
		in_errdetail = true;
		edata->detail_log = pvsprintf((n == 1 ? fmt_singular : fmt_plural), args);
		in_errdetail = false;
	}
				 ...
				 unsigned long n,...)
{
	...
	// Put guards around it in case it does.
	if (in_errdetail)  {
		write_stderr("errdetail_plural failed to print the error message\n");
		in_errdetail = false;
	} else {
		in_errdetail = true;
		edata->detail = pvsprintf((n == 1 ? fmt_singular : fmt_plural), args);
		in_errdetail = false;
	}
...
errhint(const char *fmt,...)
{
	...
	// If they do, keep track so we can bail rather than entering an infinite loop
	// when we encouter a double-fault.
	static __thread bool in_errhint = false;
	...
	// Put guards around it in case it does.
	if (in_errhint)  {
		write_stderr("errhint failed to print the error message\n");
		in_errhint = false;
	} else {
		in_errhint = true;
		edata->hint = pvsprintf(fmt, args);
		in_errhint = false;
	}
...
errcontext_msg(const char *fmt,...)
{
	...
	// If they do, keep track so we can bail rather than entering an infinite loop
	// when we encouter a double-fault.
	static __thread bool in_errcontext_msg = false;
	...
	// This could double-fault, as in spangres_ereport().
	// Put guards around it in case it does.
	if (in_errcontext_msg)  {
		write_stderr("errcontext_msg failed to print the error message\n");
		in_errcontext_msg = false;
	} else {
		in_errcontext_msg = true;
		edata->context = pvsprintf(fmt, args);
		in_errcontext_msg = false;
	}
...
format_elog_string(const char *fmt,...)
{
	...
	/* Initialize a mostly-placeholder error frame */
	edata = &errdata;
	MemSet(edata, 0, sizeof(ErrorData));
	/* the default text domain is the backend's */
