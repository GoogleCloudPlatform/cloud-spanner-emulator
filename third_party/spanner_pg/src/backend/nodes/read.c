...
/* SPANGRES BEGIN */
// Make this thread-local so deserializer is not thread-hostile.
	...
	switch ((int) type)
	{
		case LEFT_BRACE:
			...
			if (token == NULL || token[0] != '}')
				elog(ERROR, "did not find '}' at end of input node, found '%s'", token ? pnstrdup(token, tok_len) : "NULL");
			break;
		case LEFT_PAREN:
			{
				List	   *l = NIL;
				...
				else
				{
					...
					for (;;)
					{
						...
						if (token[0] == ')')
							break;
						/* SPANGRES BEGIN */
						// Guard against stack overflow due to recursion
						check_stack_depth();
						/* SPANGRES END */
