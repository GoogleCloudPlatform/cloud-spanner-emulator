...
_copyInterleaveSpec(const InterleaveSpec *from)
{
	InterleaveSpec *newnode = makeNode(InterleaveSpec);

	COPY_SCALAR_FIELD(interleavetype);
	COPY_NODE_FIELD(parent);
	COPY_SCALAR_FIELD(on_delete_action);
...
_copyRawStmt(const RawStmt *from)
{
	...
	COPY_LOCATION_FIELD(stmt_location);
	COPY_SCALAR_FIELD(stmt_len);
	COPY_NODE_FIELD(statementHints);
...
{
	AlterSpangresStatsStmt *newnode = makeNode(AlterSpangresStatsStmt);

	COPY_NODE_FIELD(package_name);
	COPY_NODE_FIELD(setstmt);
...
static Ttl *
_copyTtlSpangres(const Ttl *from)
{
	Ttl *newnode = makeNode(Ttl);
...
_copyCreateChangeStreamStmt(const CreateChangeStreamStmt *from)
{
	CreateChangeStreamStmt *newnode = makeNode(CreateChangeStreamStmt);

	COPY_NODE_FIELD(change_stream_name);
...
_copyChangeStreamTrackedTable(const ChangeStreamTrackedTable *from)
{
	ChangeStreamTrackedTable *newnode = makeNode(ChangeStreamTrackedTable);

	COPY_NODE_FIELD(table_name);
  COPY_NODE_FIELD(columns);
  COPY_SCALAR_FIELD(for_all_columns);
	return newnode;
}

static AlterChangeStreamStmt *
_copyAlterChangeStreamStmt(const AlterChangeStreamStmt *from)
{
	AlterChangeStreamStmt *newnode = makeNode(AlterChangeStreamStmt);

	COPY_NODE_FIELD(change_stream_name);
	COPY_NODE_FIELD(opt_options);
	COPY_NODE_FIELD(opt_for_tables);
  COPY_NODE_FIELD(opt_drop_for_tables);
	COPY_NODE_FIELD(opt_reset_options);
  COPY_SCALAR_FIELD(for_all);
  COPY_SCALAR_FIELD(drop_for_all);
...
copyObjectImpl(const void *from)
{
	...
	switch (nodeTag(from))
	{
		...
		case T_InterleaveSpec:
		    retval = _copyInterleaveSpec(from);
			break;
    ...
    case T_CreateChangeStreamStmt:
			retval = _copyCreateChangeStreamStmt(from);
			break;
    case T_ChangeStreamTrackedTable:
      retval = _copyChangeStreamTrackedTable(from);
      break;
    case T_AlterChangeStreamStmt:
      retval = _copyAlterChangeStreamStmt(from);
		...
		case T_AlterSpangresStatsStmt:
			retval = _copyAlterSpangresStatsStmt(from);
			break;
		case T_Ttl:
