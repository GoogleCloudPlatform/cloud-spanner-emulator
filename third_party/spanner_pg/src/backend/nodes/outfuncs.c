...
_outRangeVar(StringInfo str, const RangeVar *node)
{
	 ...
	 */
	 /* SPANGRES BEGIN */
	// Saving catalogname in output then we can deserialize the tree without
	// drop anything quietly.
...
_outIndexElem(StringInfo str, const IndexElem *node)
{
	...
	WRITE_STRING_FIELD(name);
	WRITE_NODE_FIELD(expr);
	WRITE_STRING_FIELD(expr_string);
...
_outQuery(StringInfo str, const Query *node)
{
	...
	if (node->utilityStmt)
	{
		switch (nodeTag(node->utilityStmt))
		{
			...
			case T_PrepareStmt:
			case T_ExecuteStmt:
			case T_TruncateStmt: // SPANGRES: Added support for Truncate statements.
			case T_TransactionStmt: // SPANGRES: Added support for Transaction
															// statements.
			case T_ExplainStmt:  // SPANGRES: Added support for Explain statements.
			case T_VacuumStmt:  // SPANGRES: Added support for Vacuum statements.
			case T_CreateTableAsStmt:  // SPANGRES: Added support for CTAS statements.
			case T_AlterDatabaseSetStmt: // SPANGRES: Added support for Alter
										// Database statements.
			case T_AlterSpangresStatsStmt: // SPANGRES: Added support for Alter
											// Stats statements.
			case T_AlterTableStmt: // SPANGRES: Added support for Alter Table
									// statements.
	...
	WRITE_LOCATION_FIELD(stmt_location);
	WRITE_INT_FIELD(stmt_len);
	WRITE_NODE_FIELD(statementHints);
}
...
_outA_Expr(StringInfo str, const A_Expr *node)
{
	...
	/* SPANGRES BEGIN */
	// Write the A_Expr_Kind so the deserializer will know whether to expect
	// just a name, a token after the name, a token before the name, or just a
	// token.
...
{
	/* internal representation already has leading 'b' but no quote */
	appendStringInfoChar(str, 'b');
	appendStringInfoChar(str, '"');
	const char* payload = node->bsval;
	if (payload[0] == 'b' && payload[1] != '\0')
		outToken(str, payload + 1);
	appendStringInfoChar(str, '"');
}
...
static void _outInterleaveSpec(StringInfo str, const InterleaveSpec *node) {
	WRITE_NODE_TYPE("INTERLEAVESPEC");

	WRITE_ENUM_FIELD(interleavetype, InterleaveType);
	WRITE_NODE_FIELD(parent);
	WRITE_CHAR_FIELD(on_delete_action);
	WRITE_LOCATION_FIELD(location);
}

static void _outExplainStmt(StringInfo str, const ExplainStmt *node) {
...
static void
_outAlterSpangresStatsStmt(StringInfo str, const AlterSpangresStatsStmt *node)
{
	WRITE_NODE_TYPE("ALTERSPANGRESSTATSSTMT");

	WRITE_NODE_FIELD(package_name);
	WRITE_NODE_FIELD(setstmt);
}

static void
_outTtlSpangres(StringInfo str, const Ttl *node)
{
	WRITE_NODE_TYPE("TABLETTL");

	WRITE_STRING_FIELD(name);
	WRITE_NODE_FIELD(interval);
}

static void _outCreateChangeStreamStmt(StringInfo str,
                                       const CreateChangeStreamStmt* node)
{
	WRITE_NODE_TYPE("CREATECHANGESTREAMSTMT");
	WRITE_NODE_FIELD(change_stream_name);
...
static void _outChangeStreamTrackedTable(
    StringInfo str, const ChangeStreamTrackedTable* node)
{
	WRITE_NODE_TYPE("CHANGESTREAMTRACKEDTABLE");

	WRITE_NODE_FIELD(table_name);
  WRITE_NODE_FIELD(columns);
  WRITE_BOOL_FIELD(for_all_columns);

}

static void _outAlterChangeStreamStmt(StringInfo str,
                                      const AlterChangeStreamStmt* node)
{
	WRITE_NODE_TYPE("ALTERCHANGESTREAMSTMT");
	WRITE_NODE_FIELD(change_stream_name);
  WRITE_NODE_FIELD(opt_options);
	WRITE_NODE_FIELD(opt_for_tables);
	WRITE_NODE_FIELD(opt_drop_for_tables);
	...
	else
	{
      ...
      case T_PrepareStmt:
			...
			case T_ExecuteStmt:
				...
				break;
			case T_InterleaveSpec:
				_outInterleaveSpec(str, obj);
				break;
			case T_ExplainStmt:
			...
			case T_AlterSpangresStatsStmt:
				_outAlterSpangresStatsStmt(str, obj);
				break;
			case T_Ttl:
				_outTtlSpangres(str, obj);
				break;
      case T_CreateChangeStreamStmt:
        _outCreateChangeStreamStmt(str, obj);
        break;
      case T_ChangeStreamTrackedTable:
        _outChangeStreamTrackedTable(str, obj);
        break;
        ...
        break;
			...
			default:
				...
				ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
								errmsg("statement type \"%s\" is not supported",
									   CNodeTagToNodeString(nodeTag(obj)))));
				/* SPANGRES END */
				break;
		}
		appendStringInfoChar(str, '}');
