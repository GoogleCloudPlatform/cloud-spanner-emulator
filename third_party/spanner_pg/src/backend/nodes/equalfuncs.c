...
_equalQuery(const Query *a, const Query *b)
{
	...
	COMPARE_NODE_FIELD(statementHints);
...
static bool
_equalCreateChangeStreamStmt(const CreateChangeStreamStmt *a,
							const CreateChangeStreamStmt *b)
{
	COMPARE_NODE_FIELD(change_stream_name);
...
static bool
_equalChangeStreamTrackedTable(const ChangeStreamTrackedTable *a,
						   const ChangeStreamTrackedTable *b)
{
	COMPARE_NODE_FIELD(table_name);
  COMPARE_NODE_FIELD(columns);
  COMPARE_SCALAR_FIELD(for_all_columns);

	return true;
}

static bool
_equalAlterChangeStreamStmt(const AlterChangeStreamStmt *a,
						                const AlterChangeStreamStmt *b)
{
	COMPARE_NODE_FIELD(change_stream_name);
  COMPARE_NODE_FIELD(opt_options);
  COMPARE_NODE_FIELD(opt_for_tables);
  COMPARE_NODE_FIELD(opt_drop_for_tables);
  COMPARE_NODE_FIELD(opt_reset_options);
  COMPARE_SCALAR_FIELD(for_all);
  COMPARE_SCALAR_FIELD(drop_for_all);
...
static bool
_equalInterleaveSpec(const InterleaveSpec *a, const InterleaveSpec *b)
{
	COMPARE_SCALAR_FIELD(interleavetype);
	COMPARE_NODE_FIELD(parent);
	COMPARE_SCALAR_FIELD(on_delete_action);
	COMPARE_LOCATION_FIELD(location);
...
{
	COMPARE_NODE_FIELD(package_name);
	COMPARE_NODE_FIELD(setstmt);
...
static bool
_equalTtlSpangres(const Ttl *a, const Ttl *b)
{
	COMPARE_STRING_FIELD(name);
...
equal(const void *a, const void *b)
{
    ...
    case T_CreateChangeStreamStmt:
			retval = _equalCreateChangeStreamStmt(a, b);
			break;
    case T_ChangeStreamTrackedTable:
			retval = _equalChangeStreamTrackedTable(a, b);
			break;
    case T_AlterChangeStreamStmt:
      ...
      break;
		...
		case T_InterleaveSpec:
			retval = _equalInterleaveSpec(a, b);
			break;
		case T_AlterSpangresStatsStmt:
			retval = _equalAlterSpangresStatsStmt(a, b);
			break;
		case T_Ttl:
