		...
		case 2:
			{
				...
				if (node == NULL)
				{
					...
					/* SPANGRES BEGIN */
					// Bypass consideration of table_name.name as a whole-row function
					// call. This provides a better error message in the more likely
					// table_name.bad_columm_name case.
					// TODO: Add support for pg_class entries for tables.
		...
		case 4:
			{
				 ...
				 */
				/* SPANGRES BEGIN */
				// Protect strcmp from NULLs.
				char* dbname = get_database_name(MyDatabaseId);
				if (dbname == NULL) {
					dbname = "";
				}
				if (strcmp(catname, dbname) != 0)
					/* SPANGRES END */
				{
...
transformSubLink(ParseState *pstate, SubLink *sublink)
{
	...
	/* SPANGRES BEGIN */
	// Transform join hints on this sublink.
	ListCell *lc;
	foreach (lc, sublink->joinHints) {
