...
void checkExprIsVarFree(ParseState *pstate, Node *n,
						...
						List **namespace)
{
	...
	else if (IsA(n, JoinExpr))
	{
		...
		{
			/* SPANGRES BEGIN */
			// Reject Natural joins for now. They would always join on _exists.
			ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
							errmsg("Natural join is not supported")));
			/* SPANGRES END */
		...
		/* SPANGRES BEGIN */
		// Transform join hints, if any, in place.
		ListCell *lc;
		foreach (lc, j->joinHints) {
...
buildVarFromNSColumn(ParseNamespaceColumn *nscol)
{
	...
	/* SPANGRES BEGIN */
	// Return an error if the column type is invalid.
	// TODO: Remove this change after M1.5 when Spangres returns
	// an error for unsupported column types in AddRangeTblEntrySpangres
	// instead of putting an InvalidOid in the column type list.
	if (nscol->p_vartype == InvalidOid)
			ereport(ERROR,
					(errcode(ERRCODE_UNDEFINED_COLUMN),
					 errmsg("Unable to get type of column. (Are all column "
							"types in the query supported?)")));
	/* SPANGRES END */
						   ...
						   Oid *constraint)
{
	...
	*constraint = InvalidOid;
	...
	if (infer)
	{
		...
		/*
		 * SPANGRES: resolved_unique_index_expr_UNUSED_SPANGRES has a third
		 * parameter that is unused. It was removed from the shimmed version of
		 * resolve_unique_index_expr.
		 */
		if (infer->indexElems)
