...
transformMergeStmt(ParseState *pstate, MergeStmt *stmt)
{
	 ...
	 */
	if (pstate->p_target_relation_oid != InvalidOid) {
		Relation p_target_relation =
				RelationIdGetRelation(pstate->p_target_relation_oid);
		if (p_target_relation->rd_rel->relkind != RELKIND_RELATION &&
			p_target_relation->rd_rel->relkind != RELKIND_PARTITIONED_TABLE)
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					errmsg("cannot execute MERGE on relation \"%s\"",
							RelationGetRelationName(p_target_relation)),
					errdetail_relkind_not_supported(p_target_relation->rd_rel->relkind)));
		if (p_target_relation->rd_rules != NULL &&
			p_target_relation->rd_rules->numLocks > 0)
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					errmsg("cannot execute MERGE on relation \"%s\"",
							RelationGetRelationName(p_target_relation)),
