...
static Node *makeBoolAConst(bool state, int location, core_yyscan_t yyscanner);
...
/* BEGIN SPANGRES ADDED FUNCTIONS */
static void incrementOperatorCount(int position, core_yyscan_t yyscanner);
static void incrementTypeCastCount(int position, core_yyscan_t yyscanner);
/* END SPANGRES ADDED FUNCTIONS */
...
%type <node>	stmt toplevel_stmt schema_stmt routine_body_stmt
		...
		AlterSpangresStatsStmt AlterDefaultPrivilegesStmt DefACLAction
...
%type <list>	parse_toplevel stmtmulti routine_body_stmt_list
				...
				opt_column_list opt_column_list_without_parens columnList opt_name_list
...
%type <interleavespec>	OptInterleave OptInterleaveIndex
%type <ival>		interleave_action
%type <ttlopt>    OptTtl

%type <node>    opt_change_stream_for_tables
%type <node>    change_stream_for_tables
...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	...
	INNER_P INOUT INPUT_P INSENSITIVE INSERT INSTEAD INT_P INTEGER INTERLEAVE
	...
	PARALLEL PARENT PARSER PARTIAL PARTITION PASSING PASSWORD PLACING PLANS POLICY
	...
	TRUNCATE TRUSTED TTL TYPE_P TYPES_P
...
/* BEGIN SPANGRES ADDED GRAMMAR TYPES */
%type		<list> opt_spangres_hint spangres_hint_list
%type		<defelt> spangres_hint_entry
...
stmtmulti:	stmtmulti ';' toplevel_stmt
			...
			| opt_spangres_hint toplevel_stmt
				{
					if ($2 != NULL) {
						RawStmt* stmt = makeRawStmt($2, 0);
						stmt->statementHints = $1;
						$$ = list_make1(stmt);
					}
					else
						$$ = NIL;
...
var_name:
			ColId
				{
					pg_yyget_extra(yyscanner)->nestdepth = 0;
					$$ = $1;
				}
			| var_name '.' ColId
				{
					pg_yyget_extra(yyscanner)->nestdepth++;
					if (pg_yyget_extra(yyscanner)->nestdepth > 100) {
						ereport(ERROR,
							(errcode(ERRCODE_SYNTAX_ERROR),
							errmsg("Identifier nesting too deep"),
							parser_errposition(@1)));
					}
					$$ = psprintf("%s.%s", $1, $3);
				}
...
AlterTableStmt:
		...
		|	ALTER INDEX IF_P EXISTS qualified_name alter_index_cmd
				{
					AlterTableStmt *n = makeNode(AlterTableStmt);
					n->relation = $5;
					n->cmds = list_make1($6);
...
/* `alter_index_cmd` is used to replace the `alter_table_cmds` for ALTER INDEX
 * statement because:
 *   1. DROP INCLUDE COLUMN conflicts with the DROP opt_column for ALTER TABLE.
 *   2. ALTER INDEX supports very small subset of `alter_table_cmds`.
 * With this change, ALTER INDEX <name> SET/RESET/ALTER COLUMN are not supported
 * on syntax level - customer will get an parser level error when using this
 * syntax. Note: ALTER INDEX <name> RENAME TO syntax is supported by`RenameStmt`
 * , not impacted by this change.
 */
alter_index_cmd:
			/* ALTER INDEX <name> ADD|DROP INLUDE COLUMN [IF [NOT] EXISTS] <index_name> */
			 ADD_P INCLUDE COLUMN ColId
				{
					AlterTableCmd *n = makeNode(AlterTableCmd);
					n->subtype = AT_AddIndexIncludeColumn;
					n->missing_ok = false;
					n->name = $4;
					$$ = (Node *)n;
				}
			| ADD_P INCLUDE COLUMN IF_P NOT EXISTS ColId
				{
					AlterTableCmd *n = makeNode(AlterTableCmd);
					n->subtype = AT_AddIndexIncludeColumn;
					n->missing_ok = true;
      ...
      | DROP INCLUDE COLUMN ColId
				{
					AlterTableCmd *n = makeNode(AlterTableCmd);
					n->subtype = AT_DropIndexIncludeColumn;
					n->missing_ok = false;
					n->name = $4;
					$$ = (Node *)n;
				}
      | DROP INCLUDE COLUMN IF_P EXISTS ColId
				{
					AlterTableCmd *n = makeNode(AlterTableCmd);
					n->subtype = AT_DropIndexIncludeColumn;
					n->missing_ok = true;
...
alter_table_cmd:
			...
			/*
			* We need to store the original expression string and translate it later
			* to the GoogleSQL expression. Here, we do not add new grammar rules but
			* make two separated versions of the existing rule. We can only extract
			* the original strings in this way.
			*/
			/* ALTER TABLE <name> ALTER [COLUMN] <colname> SET DEFAULT <expr> */
      ...
      | SET ON DELETE_P CASCADE
				{
					AlterTableCmd *n = makeNode(AlterTableCmd);
					n->subtype = AT_SetOnDeleteCascade;
					$$ = (Node *)n;
				}
      | SET ON DELETE_P NO ACTION
				{
					AlterTableCmd *n = makeNode(AlterTableCmd);
					n->subtype = AT_SetOnDeleteNoAction;
					$$ = (Node *)n;
				}
...
OptTtl: TTL ConstInterval Sconst opt_interval ON ColId
                {
					Ttl *n = makeNode(Ttl);
					n->name = $6;
					TypeName *t = $2;
...
CreateStmt:	CREATE OptTemp TABLE qualified_name '(' OptTableElementList ')'
		...
		| CREATE OptTemp TABLE qualified_name PARTITION OF qualified_name
			...
			table_access_method_clause OptWith OnCommitOption OptTableSpace OptInterleave OptTtl
				{
					CreateStmt *n = makeNode(CreateStmt);
					$4->relpersistence = $2;
					n->relation = $4;
			...
			| CHECK '(' a_expr ')' opt_no_inherit
				{
					...
					int expr_start = @3;
					int expr_end  = @4;
					n->constraint_expr_string = pnstrdup(pg_yyget_extra(yyscanner)->core_yy_extra.scanbuf + expr_start, expr_end - expr_start);
...
opt_column_list_without_parens:
		  columnList  						{ $$ = $1; }
			| /*EMPTY*/								{ $$ = NIL; }
		;
...
OptInterleave: INTERLEAVE IN_P qualified_name
		...
		| INTERLEAVE IN_P PARENT qualified_name interleave_action
			{
				InterleaveSpec *n = makeNode(InterleaveSpec);
				n->interleavetype = INTERLEAVE_IN_PARENT;
				n->parent = $4;
				n->on_delete_action = $5;
				n->location = @1;
				$$ = n;
			}
		| /*EMPTY*/								{ $$ = NULL; }

OptInterleaveIndex: INTERLEAVE IN_P qualified_name
			{
				InterleaveSpec *n = makeNode(InterleaveSpec);
				n->interleavetype = INTERLEAVE_IN;
				n->parent = $3;
...
interleave_action: key_delete	{ $$ = $1; }
		| /*EMPTY*/								{ $$ = FKCONSTR_ACTION_NOACTION; }
...
AlterSpangresStatsStmt:
			ALTER STATISTICS any_name SET OPTIONS '(' generic_set ')'
				{
					AlterSpangresStatsStmt *n = makeNode(AlterSpangresStatsStmt);
					n->package_name = $3;
					n->setstmt = $7;
					$$ = (Node *)n;
				}
...
SeqOptElem: AS SimpleTypename
			...
			| BIT_REVERSED_POSITIVE
			    {
					$$ = makeDefElem("bit_reversed_positive", (Node *)makeInteger(true), @1);
				}
			| CACHE NumericOnly
				{
			...
			| START COUNTER opt_with NumericOnly
				{
					$$ = makeDefElem("start_counter", (Node *)$4, @1);
				}
			| RESTART
				{
					$$ = makeDefElem("restart", NULL, @1);
...
DropStmt:	DROP object_type_any_name IF_P EXISTS any_name_list opt_drop_behavior
      ...
      | DROP CHANGE STREAM qualified_name
				{
					DropStmt *n = makeNode(DropStmt);
...
privilege_target:
			...
			| CHANGE STREAM qualified_name_list
				{
					PrivTarget *n = (PrivTarget *) palloc(sizeof(PrivTarget));
					n->targtype = ACL_TARGET_OBJECT;
					n->objtype = OBJECT_CHANGE_STREAM;
					n->objs = $3;
					$$ = n;
				}
			...
			| ALL CHANGE STREAMS IN_P SCHEMA name_list
				{
					PrivTarget *n = (PrivTarget *) palloc(sizeof(PrivTarget));
					n->targtype = ACL_TARGET_ALL_IN_SCHEMA;
					n->objtype = OBJECT_CHANGE_STREAM;
...
IndexStmt:	CREATE opt_unique INDEX opt_concurrently opt_index_name
			...
			opt_include opt_reloptions OptTableSpace OptInterleaveIndex where_clause
				{
...
CreateChangeStreamStmt:
			CREATE CHANGE STREAM qualified_name opt_change_stream_for_tables opt_definition
				{
					CreateChangeStreamStmt *n = makeNode(CreateChangeStreamStmt);
					n->change_stream_name = $4;
					if ($5 != NULL)
					{
						/* FOR */
						if (IsA($5, List))
							n->opt_for_tables = (List *)$5;
						/* FOR ALL */
						else
							n->for_all = true;
					}
...
opt_change_stream_for_tables:
			change_stream_for_tables					{ $$ = $1; }
			| /* EMPTY */							{ $$ = NULL; }
		;

change_stream_for_tables:
			FOR change_stream_tracked_tables_list
				{
					$$ = (Node *) $2;
				}
			| FOR ALL
...
change_stream_tracked_tables_list:
			change_stream_tracked_tables
      { $$ = list_make1($1); }
			| change_stream_tracked_tables_list ',' change_stream_tracked_tables
      { $$ = lappend($1, $3); }
		;

change_stream_tracked_tables:
  qualified_name '(' opt_column_list_without_parens ')'
  {
    ChangeStreamTrackedTable *n = makeNode(ChangeStreamTrackedTable);
    n->table_name = $1;
    if ($3 != NULL) {
      n->columns = $3;
    }
  ...
  | qualified_name
  {
    ChangeStreamTrackedTable *n = makeNode(ChangeStreamTrackedTable);
    n->table_name = $1;
    n->for_all_columns = true;
    $$ = (Node *)n;
  }

/*****************************************************************************
 *
 * ALTER CHANGE STREAM name SET ( change_stream_for_clause )
 *
 * ALTER CHANGE STREAM name SET ( options )
 *
 * ALTER CHANGE STREAM name DROP FOR ALL
 *
 * ALTER CHANGE STREAM name RESET ( options )
 *
 *****************************************************************************/

 AlterChangeStreamStmt:
       ALTER CHANGE STREAM qualified_name SET change_stream_for_tables
         {
           AlterChangeStreamStmt *n = makeNode(AlterChangeStreamStmt);
           n->change_stream_name = $4;
           /* FOR */
            if (IsA($6, List))
              n->opt_for_tables = (List *)$6;
           /* FOR ALL */
           else
              n->for_all = true;
           $$ = (Node *)n;
         }
       | ALTER CHANGE STREAM qualified_name SET definition
         {
				   AlterChangeStreamStmt *n = makeNode(AlterChangeStreamStmt);
				   n->change_stream_name = $4;
					 n->opt_options = $6;
       ...
       | ALTER CHANGE STREAM qualified_name DROP change_stream_for_tables
         {
           AlterChangeStreamStmt *n = makeNode(AlterChangeStreamStmt);
           n->change_stream_name = $4;
           if ($6 != NULL)
       ...
       | ALTER CHANGE STREAM qualified_name RESET '(' name_list ')'
         {
           AlterChangeStreamStmt *n = makeNode(AlterChangeStreamStmt);
           n->change_stream_name = $4;
		...
		| CYCLE columnList SET ColId USING ColId
			{
				...
				n->cycle_col_list = $2;
				n->cycle_mark_column = $4;
				n->cycle_mark_value = makeBoolAConst(true, -1, yyscanner);
				n->cycle_mark_default = makeBoolAConst(false, -1, yyscanner);
...
joined_table:
			...
			| table_ref CROSS JOIN opt_spangres_hint table_ref
				{
			...
			| table_ref join_type JOIN opt_spangres_hint table_ref join_qual
				{
					JoinExpr *n = makeNode(JoinExpr);
					n->jointype = $2;
					n->isNatural = false;
					...
					{
						 /* USING clause */
						n->usingClause = linitial_node(List, castNode(List, $6));
						n->join_using_alias = lsecond_node(Alias, castNode(List, $6));
					}
					else
					{
						/* ON clause */
			...
			| table_ref JOIN opt_spangres_hint table_ref join_qual
				{
...
relation_expr:
			...
			| ONLY qualified_name opt_spangres_hint
				{
					/* no inheritance */
					$$ = $2;
					$$->inh = false;
 ...
 * SPANGRES: aliasing for INT and INTEGER has been changed to point to INT8
 */
Numeric:	INT_P
				{
					$$ = SystemTypeName("int8");
...
a_expr:		c_expr
		 ...
		 */
			| '+' a_expr					%prec UMINUS
				{
					incrementOperatorCount(@1, yyscanner);
					$$ = (Node *) makeSimpleA_Expr(AEXPR_OP, "+", NULL, $2, @1);
				}
			| '-' a_expr					%prec UMINUS
				{
			...
			| a_expr IN_P opt_spangres_hint in_expr
				{
					...
					else
					{
						if ($3 != NIL) {
							ereport(ERROR,
								(errcode(ERRCODE_SYNTAX_ERROR),
								 errmsg("HINTs cannot be specified on IN clause with value list")));
						}
						/* generate scalar IN expression */
						$$ = (Node *) makeSimpleA_Expr(AEXPR_IN, "=", $1, $4, @2);
					}
				}
			| a_expr NOT_LA IN_P opt_spangres_hint in_expr						%prec NOT_LA
				{
					/* in_expr returns a SubLink or a list of a_exprs */
					if (IsA($5, SubLink))
					{
					...
					else
					{
						...
						$$ = (Node *) makeSimpleA_Expr(AEXPR_IN, "<>", $1, $5, @2);
					}
				}
			...
			| XMLPARSE '(' document_or_content a_expr xml_whitespace_option ')'
				{
					XmlExpr *x = (XmlExpr *)
						makeXmlExpr(IS_XMLPARSE, NULL, NIL,
									list_make2($4, makeBoolAConst($5, -1, yyscanner)),
									@1);
					x->xmloption = $3;
					$$ = (Node *)x;
				}
			...
			| a_expr FOR a_expr
				{
					 ...
					 */
					// SPANGRES: We changed this from int4 to int8 to match the signature
					// that we also changed.
					$$ = list_make3($1, makeIntConst(1, -1),
									makeTypeCast($3,
												 SystemTypeName("int8"), -1, yyscanner));
				}
...
columnref:	ColId
				{
					ColumnRef *c = (ColumnRef *)makeColumnRef($1, NIL, @1, yyscanner);
					if (list_length(c->fields) > 100) {
						ereport(ERROR,
								(errcode(ERRCODE_SYNTAX_ERROR),
								 errmsg("Too many fields in the colunref"),
								 parser_errposition(@1)));
					}
					$$ = (Node *)c;
				}
			| ColId indirection
			    {
                    ColumnRef *c = (ColumnRef *)makeColumnRef($1, $2, @1, yyscanner);
					if (list_length(c->fields) > 100) {
						ereport(ERROR,
								(errcode(ERRCODE_SYNTAX_ERROR),
								 errmsg("Too many fields in the colunref"),
...
AexprConst: Iconst
			...
			| func_name '(' func_arg_list opt_sort_clause ')' Sconst
				{
					...
					t->typmods = $3;
					t->location = @1;
					$$ = makeStringConstCast($6, @6, t, yyscanner);
				}
			...
			| TRUE_P
				{
					$$ = makeBoolAConst(true, @1, yyscanner);
				}
			| FALSE_P
				{
					$$ = makeBoolAConst(false, @1, yyscanner);
				}
			| NULL_P
				{
					$$ = makeNullAConst(@1);
...
opt_spangres_hint:
	SPANGRES_HINT_START spangres_hint_list SPANGRES_HINT_END { $$ = $2; }
	| /* EMPTY */					{ $$ = NIL; }
	;

spangres_hint_list:
	spangres_hint_entry	{ $$ = list_make1($1); }
...
makeTypeCast(Node *arg, TypeName *typename, int location,
			 core_yyscan_t yyscanner)
{
	incrementTypeCastCount(location, yyscanner);
					...
					core_yyscan_t yyscanner)
{
	...
	return makeTypeCast(s, typename, -1, yyscanner);
}
 ...
 */
static Node *
makeBoolAConst(bool state, int location, core_yyscan_t yyscanner)
{
	A_Const *n = makeNode(A_Const);
	...
	return makeTypeCast((Node *)n, SystemTypeName("bool"), -1, yyscanner);
}
...
makeXmlExpr(XmlExprOp op, char *name, List *named_args, List *args,
			int location)
{
	/* There are no plans to support XML expressions in spangres.
	* Furthermore, erroring out in the parser will prevent bugs from
	* appearing in ASAN runs of the fuzzer (b/284718081).
	*/
	ereport(ERROR,
		(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
		errmsg("XML expressions are not supported")));
...
parser_init(base_yy_extra_type *yyext)
{
	...
	yyext->operator_count = 0;
	yyext->type_cast_count = 0;
}

/* BEGIN SPANGRES ADDED FUNCTIONS */
static void
incrementOperatorCount(int position, core_yyscan_t yyscanner) {
	pg_yyget_extra(yyscanner)->operator_count++;
	if (pg_yyget_extra(yyscanner)->operator_count > 1000) {
		ereport(ERROR,
				(errcode(ERRCODE_SYNTAX_ERROR),
				 errmsg("Too many operators in the statement"),
				 parser_errposition(position)));
	}
}

static void
incrementTypeCastCount(int position, core_yyscan_t yyscanner) {
	pg_yyget_extra(yyscanner)->type_cast_count++;
	if (pg_yyget_extra(yyscanner)->type_cast_count > 1000) {
		ereport(ERROR,
				(errcode(ERRCODE_SYNTAX_ERROR),
				 errmsg("Too many type casts in the statement"),
				 parser_errposition(position)));
	}
}
