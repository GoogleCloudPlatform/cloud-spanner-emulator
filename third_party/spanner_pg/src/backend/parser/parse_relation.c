					...
					int sublevels_up, const char *colname, int location)
{
	...
	if (attnum > InvalidAttrNumber)
	{
		...
		// TODO: Remove this change after M1.5 when Spangres
		// returns an error for unsupported column types in
		// AddRangeTblEntrySpangres instead of putting an InvalidOid in the
		// column type list.
		if (nscol->p_vartype == InvalidOid)
			ereport(ERROR,
					(errcode(ERRCODE_UNDEFINED_COLUMN),
					 errmsg("Unable to get type of column \"%s\" of relation "
							"\"%s\". (Is the column type supported?)",
							colname,
							rte->eref->aliasname)));
		/* SPANGRES END */
				   ...
				   bool inFromCl)
{
  ...
  GetColumnTypesC(
	...
	ParseNamespaceItem* nsitem = buildNSItemFromLists(
