...
store_match(pg_ctype_cache *pcc, pg_wchar chr1, int nchrs)
{
	...
	if (nchrs > 1)
	{
		if (pcc->cv.nranges >= pcc->cv.rangespace)
		{
			pcc->cv.rangespace *= 2;
			// SPANGRES BEGIN
			newchrs = (chr *) repalloc(pcc->cv.ranges,
	...
	else
	{
		...
		if (pcc->cv.nchrs >= pcc->cv.chrspace)
		{
			pcc->cv.chrspace *= 2;
			// SPANGRES BEGIN
			newchrs = (chr *) repalloc(pcc->cv.chrs,
...
// SPANGRES BEGIN
// All memory allocation is changed to use palloc*, repalloc* instead of malloc,
// realloc. Because the allocated memory is only valid for the lifetime of
// the underlying PGArena memory context, we do not cache the result, nor do we
// perform cache lookups before executing this function.
static struct cvec *
pg_ctype_get_cache(pg_wc_probefunc probefunc, int cclasscode)
{
	 ...
	 *
	 * Cache lookup is skipped in the custom implementation
	 */

	pcc = (pg_ctype_cache *) palloc(sizeof(pg_ctype_cache));
	if (pcc == NULL)
		return NULL;
	...
	pcc->cv.nchrs = 0;
	pcc->cv.chrspace = 128;
	pcc->cv.chrs = (chr *) palloc(pcc->cv.chrspace * sizeof(chr));
	pcc->cv.nranges = 0;
	pcc->cv.rangespace = 64;
	pcc->cv.ranges = (chr *) palloc(pcc->cv.rangespace * sizeof(chr) * 2);
	 ...
	 *
	 * Cache saving is skipped in the custom implementation
	 */
...
static struct cvec *
// SPANGRES BEGIN
pg_ctype_get_cache_UNUSED_SPANGRES(pg_wc_probefunc probefunc, int cclasscode)
// SPANGRES END
{
	pg_ctype_cache *pcc;
	pg_wchar	max_chr;
