...
bool SectionMemoryManager::hasSpace(const MemoryGroup &MemGroup,
  ...
  for (const FreeMemBlock &FreeMB : MemGroup.FreeMem) {
    if (FreeMB.Free.allocatedSize() >= Size) return true;
  }
  return false;
}
...
void SectionMemoryManager::reserveAllocationSpace(
    uintptr_t CodeSize, uint32_t CodeAlign_i, uintptr_t RODataSize,
    uint32_t RODataAlign_i, uintptr_t RWDataSize, uint32_t RWDataAlign_i) {
...
uint8_t *SectionMemoryManager::allocateSection(
  ...
  // been initialized.
  if (CodeMem.Near.base() == nullptr) CodeMem.Near = MB;
  if (RODataMem.Near.base() == nullptr) RODataMem.Near = MB;
  if (RWDataMem.Near.base() == nullptr) RWDataMem.Near = MB;
...
std::error_code SectionMemoryManager::applyMemoryGroupPermissions(
    MemoryGroup &MemGroup, unsigned Permissions) {
...
class DefaultMMapper final : public SectionMemoryManager::MemoryMapper {
 public:
  sys::MemoryBlock allocateMappedMemory(
      SectionMemoryManager::AllocationPurpose Purpose, size_t NumBytes,
      const sys::MemoryBlock *const NearBlock, unsigned Flags,
