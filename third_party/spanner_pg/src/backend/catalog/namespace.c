						 ...
						 RangeVarGetRelidCallback callback, void *callback_arg)
{
	...
	if (relation->catalogname)
	{
			...
			if (strcmp(relation->catalogname, dbname) != 0)
			/* SPANGRES END */
			ereport(ERROR,
...
RangeVarGetCreationNamespace(const RangeVar *newRelation)
{
	...
	if (newRelation->catalogname)
	{
			...
			if (strcmp(newRelation->catalogname, dbname) != 0)
			/* SPANGRES END */
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					 errmsg("cross-database references are not implemented: \"%s.%s.%s\"",
	...
	for (int proc_index = 0; proc_index < proc_count; ++proc_index) {
		...
		if (argnumbers)
		{
			...
			for (int j = 0; j < pronargs; j++)
				// SPANGRES BEGIN
				newResult->args[j] = p_argtypes[argnumbers[j]];
				// SPANGRES END
		}
		else
		{
						 ...
						 char **objname_p)
{
	...
	switch (list_length(names))
	{
		...
		case 3:
			...
			if (strcmp(catalogname, dbname) != 0)
			/* SPANGRES END */
				ereport(ERROR,
						(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
