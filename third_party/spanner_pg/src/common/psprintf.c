 ...
 *
 * pvsprintf is equivalent to psprintf except that its format-string arguments
 * are passed as a va_list rather than as variadic function arguments.
 */
char*
pvsprintf(const char* fmt, va_list args)
{
	...
	for (;;)
	{
		...
		va_list args_copy;  /* Copy args so we can reuse it for each iteration */
		va_copy(args_copy, args);
		newlen = pvsnprintf(result, len, fmt, args_copy);
		va_end(args_copy);
