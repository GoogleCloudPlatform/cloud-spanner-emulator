...
typedef struct RangeVar
{
	...
	List	   *tableHints;			/* list of DefElem nodes for table hints */
} RangeVar;
...
typedef struct SubLink
{
	...
	int			location;		/* token location, or -1 if unknown */
	/* BEGIN SPANGRES ADDITIONAL FIELDS */
	List	   *joinHints;		/* list of DefElem nodes containing join hints for a
													 * WHERE <x> IN (<subquery>) expression. */
	/* END SPANGRES */
} SubLink;
...
typedef struct JoinExpr
{
	...
	/* BEGIN SPANGRES ADDITIONAL FIELDS */
	List	   *joinHints;	/* list of DefElem nodes for hints on this join */
	/* END SPANGRES */
} JoinExpr;
