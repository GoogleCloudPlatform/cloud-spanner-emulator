...
typedef struct Query
{
	...
	List	   *statementHints;	/* list of DefElem nodes containing statement-level
...
/*
 * InterleaveSpec - parse-time representation of an interleave in specification
 *
 * This represents interleaved table information
 */
typedef struct InterleaveSpec {
	NodeTag				type;
	InterleaveInType	interleavetype;
	RangeVar			*parent;	/* Parent table name */
        char on_delete_action;
        int					location;
} InterleaveSpec;

/*
 * TTL - TTL specification for a table
 *
...
{
	NodeTag type;
	char *name; /* Column the TTL applies to */
	Node *interval; /* Interval spec for TTL */
} Ttl;
...
typedef struct RangeTblEntry
{
	...
	/* BEGIN SPANGRES ADDITIONAL FIELDS */
	List	   *tableHints;		/* DefElem nodes containing table hints, applies
								 * to RTE_RELATION only. For RTE_JOIN, the hints
								 * are on the corresponding JoinExpr */
} RangeTblEntry;
...
/* ----------------------
 *		Alter Statistics Package Statement
 * ----------------------
 */
typedef struct AlterSpangresStatsStmt
{
	NodeTag		type;
	List	   *package_name;		/* qualified statistics package name */
