...
/* SPANGRES BEGIN */
// Make this global a thread-local and add a public Clear() function to clean up
// after ourselves.
// We need the ability to clean up (and PostgreSQL didn't) because Spangres runs
// through cycles of setup/teardown and can't leave state around in between.
// PostgreSQL uses the nullness of this pointer to tell whether it needs to
// perform setup, so this cleanup step prompts a new setup the next iteration.
static __thread HTAB *timezone_cache = NULL;
void ClearTimezoneHashtablePointer() { timezone_cache = NULL; }
bool TimezoneCleared() { return timezone_cache == NULL; }
/* SPANGRES END */
...
init_timezone_hashtable(void)
{
  ...
  /* SPANGRES BEGIN */
  // Create the hash table from CurrentMemoryContext instead of Top.
  // We're making an exception to our usual shim-whole-function policy
  // here because the PostgreSQL timezone library isn't part of the
  // postgres/includes, postgres/src mono-library. It's a separate library
  // with a public and a private header. Exporting the dependencies of
  // this function to shim it out entirely means significaly restructuring
  // the library. This 2-line change has a much smaller impact on code
  // upgradability.
  // To make this change we set the HASH_CONTEXT bit in the options flag so that
  // hash creation logic uses the .hcxt field of the options struct for the host
  // memory context.
