...
TEST_P(IdentityColumnTest, NamedSchema) {
  if (GetParam() == database_api::DatabaseDialect::POSTGRESQL) {
    GOOGLESQL_ASSERT_OK(UpdateSchema({"CREATE SCHEMA s",
                            R"(
                            CREATE TABLE s.t (
                              id BIGINT GENERATED BY DEFAULT AS IDENTITY (
...
TEST_P(IdentityColumnTest, SameTableColumnNameDifferentSchemas) {
  if (GetParam() == database_api::DatabaseDialect::POSTGRESQL) {
    GOOGLESQL_ASSERT_OK(UpdateSchema({"CREATE SCHEMA s1",
                            R"(
        CREATE TABLE s1.test_table (
          ...
          value BIGINT,
          PRIMARY KEY(id)))"}));
    GOOGLESQL_ASSERT_OK(UpdateSchema({"CREATE SCHEMA s2",
                            R"(
        CREATE TABLE s2.test_table (
          ...
          value BIGINT,
          PRIMARY KEY(id)))"}));
    GOOGLESQL_ASSERT_OK(Insert("s1.test_table", {"value"}, {1}));
    GOOGLESQL_ASSERT_OK(Insert("s1.test_table", {"value"}, {2}));
    GOOGLESQL_ASSERT_OK(Insert("s1.test_table", {"value"}, {3}));
    ASSERT_THAT(
        Query(
            "SELECT "
            "spanner.GET_TABLE_COLUMN_IDENTITY_STATE('s1.test_table.id') >= 3"),
        IsOkAndHoldsRows({{true}}));
    GOOGLESQL_ASSERT_OK(Insert("s2.test_table", {"value"}, {1}));
    GOOGLESQL_ASSERT_OK(Insert("s2.test_table", {"value"}, {2}));
    GOOGLESQL_ASSERT_OK(Insert("s2.test_table", {"value"}, {3}));
    ASSERT_THAT(
        Query(
            "SELECT "
...
TEST_P(IdentityColumnTest, DropDefault) {
  ...
  GOOGLESQL_ASSERT_OK(UpdateSchema({"ALTER TABLE t ALTER COLUMN id DROP DEFAULT"}));
  GOOGLESQL_ASSERT_OK(
      CommitDml({SqlStatement("INSERT INTO t(value) VALUES (1), (2), (3)")}));
  // Check that counter has incremented at least 3 times.
  if (GetParam() == database_api::DatabaseDialect::POSTGRESQL) {
...
TEST_P(IdentityColumnTest, DropTable) {
  GOOGLESQL_ASSERT_OK(UpdateSchema({"DROP TABLE t"}));
}

TEST_P(IdentityColumnTest, DropIdentityColumn) {
  GOOGLESQL_ASSERT_OK(UpdateSchema({"ALTER TABLE t DROP COLUMN non_key_col"}));
}
...
TEST_P(IdentityColumnTest, IdentityColumnFunctionInvalidCases) {
  if (GetParam() == database_api::DatabaseDialect::POSTGRESQL) {
    GOOGLESQL_ASSERT_OK(UpdateSchema({R"(CREATE SCHEMA Sch)",
                            R"(
      CREATE TABLE Sch.T (
        id BIGINT GENERATED BY DEFAULT AS IDENTITY (
...
TEST_P(IdentityColumnTest, AddColumnBackfillWithNonNullValues) {
  GOOGLESQL_ASSERT_OK(
      CommitDml({SqlStatement("INSERT INTO t(value) VALUES (1), (2), (3)")}));
  if (GetParam() == database_api::DatabaseDialect::POSTGRESQL) {
    GOOGLESQL_ASSERT_OK(UpdateSchema({R"(ALTER TABLE t ADD COLUMN another_id BIGINT
    ...
    GOOGLESQL_ASSERT_OK(UpdateSchema({R"(ALTER TABLE t ADD COLUMN another_id INT64
            GENERATED BY DEFAULT AS IDENTITY (
            BIT_REVERSED_POSITIVE START COUNTER WITH 1000))"}));
    GOOGLESQL_ASSERT_OK(
        CommitDml({SqlStatement("INSERT INTO t(value) VALUES (4), (5), (6)")}));
...
TEST_P(IdentityColumnTest, AlterEntireIdentityColumn) {
  if (GetParam() == database_api::DatabaseDialect::POSTGRESQL) {
    ...
    GOOGLESQL_ASSERT_OK(UpdateSchema({R"(ALTER TABLE t ALTER COLUMN id INT64
            GENERATED BY DEFAULT AS IDENTITY (
            BIT_REVERSED_POSITIVE START COUNTER WITH 1000))"}));
...
TEST_P(IdentityColumnTest, AlterIdentityColumnRestartCounter) {
  if (GetParam() == database_api::DatabaseDialect::POSTGRESQL) {
    ...
    GOOGLESQL_ASSERT_OK(UpdateSchema({R"(ALTER TABLE T ALTER COLUMN id
