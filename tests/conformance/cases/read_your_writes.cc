...
class ReadYourWritesTest
 ...
 public:
  absl::Status SetUpDatabase() override {
    return SetSchemaFromFile("read_your_writes.test");
  }
};

INSTANTIATE_TEST_SUITE_P(
    PerDialectReadYourWritesTest, ReadYourWritesTest,
    testing::Values(database_api::DatabaseDialect::GOOGLE_STANDARD_SQL,
                    database_api::DatabaseDialect::POSTGRESQL),
    [](const testing::TestParamInfo<ReadYourWritesTest::ParamType>& info) {
      return database_api::DatabaseDialect_Name(info.param);
    });

TEST_P(ReadYourWritesTest, CanReadAlreadyCommittedTimestampKey) {
...
TEST_P(ReadYourWritesTest, CanReadExplictlySetTimestampKey) {
...
TEST_P(ReadYourWritesTest, CannotReadPendingCommitTimestampInKey) {
  // Perform writes and reads using the same transaction.
  auto txn = Transaction(Transaction::ReadWriteOptions());
  if (dialect_ == database_api::POSTGRESQL) {
    GOOGLESQL_ASSERT_OK(ExecuteDmlTransaction(
        txn, SqlStatement("INSERT INTO CommitTsTable (id, ts, val) "
                          "VALUES ($1, SPANNER.PENDING_COMMIT_TIMESTAMP(), $2)",
                          {{"p1", Value("id1")}, {"p2", Value("val1")}})));
  } else {
    GOOGLESQL_ASSERT_OK(ExecuteDmlTransaction(
        txn, SqlStatement("INSERT INTO CommitTsTable (id, ts, val) "
...
TEST_P(ReadYourWritesTest, CannotQueryTableWithPendingCommitTimestampKey) {
...
TEST_P(ReadYourWritesTest, CannotReadTableWithPendingCommitTimestampKey) {
...
TEST_P(ReadYourWritesTest,
  ...
  if (dialect_ == database_api::POSTGRESQL) {
    GOOGLESQL_ASSERT_OK(ExecuteDmlTransaction(
        txn,
        SqlStatement("INSERT INTO CommitTsChildTable (id, ts, ts2, val2) "
                     "VALUES ($1, $2, SPANNER.PENDING_COMMIT_TIMESTAMP(), $3)",
                     {{"p1", Value("id1")},
                      {"p2", Value(explicit_ts)},
                      {"p3", Value("val22")}})));
  } else {
...
TEST_P(ReadYourWritesTest, CanReadAlreadyCommittedTimestampColumn) {
...
TEST_P(ReadYourWritesTest, CanReadExplictlySetTimestampColumn) {
  // Perform writes and reads using the same transaction.
  auto txn = Transaction(Transaction::ReadWriteOptions());
...
TEST_P(ReadYourWritesTest, CannotReadPendingCommitTimestampInColumn) {
  // Perform writes and reads using the same transaction.
  auto txn = Transaction(Transaction::ReadWriteOptions());
  if (dialect_ == database_api::POSTGRESQL) {
    GOOGLESQL_ASSERT_OK(ExecuteDmlTransaction(
        txn, SqlStatement("INSERT INTO NonKeyCommitTsTable (id, key, ts) "
                          "VALUES ($1, $2, SPANNER.PENDING_COMMIT_TIMESTAMP())",
                          {{"p1", Value("id1")}, {"p2", Value("key1")}})));
  } else {
    GOOGLESQL_ASSERT_OK(ExecuteDmlTransaction(
        txn, SqlStatement("INSERT INTO NonKeyCommitTsTable (id, key, ts) "
...
TEST_P(ReadYourWritesTest, CannnotReadNonKeyColumnWithPendingCommitTimestamp) {
  // Perform writes and reads using the same transaction.
  auto txn = Transaction(Transaction::ReadWriteOptions());
...
TEST_P(ReadYourWritesTest, CanReadBufferedUpdatesToIndex) {
  ...
  if (dialect_ == database_api::POSTGRESQL) {
    EXPECT_THAT(
        Query(
            "SELECT userid, email FROM Users /* @force_index=UsersByEmail */"),
...
TEST_P(ReadYourWritesTest, CannotViolateUniqueIndexConstraint) {
  auto txn = Transaction(Transaction::ReadWriteOptions());

  // Insert: 111 - a@foo.com
  GOOGLESQL_EXPECT_OK(ExecuteDmlTransaction(
      txn,
